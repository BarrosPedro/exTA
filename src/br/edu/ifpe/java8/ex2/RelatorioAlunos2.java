...
public class RelatorioAlunos2 {
	...
	private static final Map<TipoRelatorio2, Function<Aluno, String>> FUNCTION;
	static {
		FUNCTION = new HashMap<>();
		FUNCTION.put(TipoRelatorio2.NOME, alunos -> alunos.getNome());
		FUNCTION.put(TipoRelatorio2.NOME_MATRICULA, alunos -> alunos.getNome() + SEPARADOR +  alunos.getMatricula());
		FUNCTION.put(TipoRelatorio2.NOME_MATRICULA_MEDIA, alunos -> alunos.getNome() + SEPARADOR +  alunos.getMatricula() + SEPARADOR + alunos.getMedia());
		FUNCTION.put(TipoRelatorio2.MATRICULA_MEDIA, alunos -> alunos.getMatricula() + SEPARADOR +  alunos.getMedia());
		FUNCTION.put(TipoRelatorio2.NOME_MEDIA, alunos -> alunos.getNome() + SEPARADOR +  alunos.getMedia());
	...
	public static void gerar(TipoRelatorio2 opcao) {
		...
		try {
			...
			while (leitor.hasNextLine()) {
				...
				escritor.println(FUNCTION.get(opcao).apply(aluno));
